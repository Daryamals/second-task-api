...
public class EntitySteps {
	...
	public int createEntity(EntityRequest entity) {
		return given().contentType(ContentType.JSON).body(entity).when().post(CREATE).then()
				.statusCode(OK.getStatusCode()).extract().as(Integer.class);
	}
	...
	public EntityResponse[] getAllEntities(int page, int perPage) {
		return given().queryParam("page", page).queryParam("perPage", perPage).when().get(GET_ALL).then()
	...
	public int getEntityStatus(int entityId) {
		return given().pathParam("id", entityId).when().get(GET).then().extract().statusCode();
	}
}
